...
void  Crx(CAN_HandleTypeDef*  CANx_Handle);
...
{  
	...
	*/
	/*
	gRxCAN.ID = (uint16_t)(CANx_Handle->pRxMsg->StdId);
	//RxMSG.rtr = CANx_Handle->pRxMsg->RTR;
	gRxCAN.LEN = CANx_Handle->pRxMsg->DLC;
	for(i=0;i<gRxCAN.LEN;i++)
	{
		gRxCAN.BUF[i] = CANx_Handle->pRxMsg->Data[i];
	} 
...
void CAN2_RX0_IRQHandler(void)
{
	//unsigned int i = 0;
	//INT8U err;
	CAN_HandleTypeDef*  CANx_Handle=&CAN2_HandleStructure;
	//Message RxMSG = Message_Initializer;
	gMessagePending = 1;
	Crx(CANx_Handle);
...
void  Crx(CAN_HandleTypeDef*  CANx_Handle)
{
	...
	for(i = 0; i < gMessagePending; i++)
	{
		...
		gRxCAN.ID=(uint16_t)(CANx_Handle->pRxMsg->StdId);
		gRxCAN.LEN=CANx_Handle->pRxMsg->DLC;
		gRxCAN.BUF[0] = CANx_Handle->pRxMsg->Data[0];
		gRxCAN.BUF[1] = CANx_Handle->pRxMsg->Data[1];
		gRxCAN.BUF[2] = CANx_Handle->pRxMsg->Data[2];
		gRxCAN.BUF[3] = CANx_Handle->pRxMsg->Data[3];
		gRxCAN.BUF[4] = CANx_Handle->pRxMsg->Data[4];
		gRxCAN.BUF[5] = CANx_Handle->pRxMsg->Data[5];
		gRxCAN.BUF[6] = CANx_Handle->pRxMsg->Data[6];
		gRxCAN.BUF[7] = CANx_Handle->pRxMsg->Data[7];
		CanOpenRxPres();
	}
	gMessagePending = 0;
}
