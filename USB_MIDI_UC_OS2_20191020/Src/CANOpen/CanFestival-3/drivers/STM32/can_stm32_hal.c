...
unsigned char canSend(CAN_PORT CANx, Message *m)	                
{
		...
		CAN_HandleTypeDef*  CANx_Handle;
		/*
		u32 CAN_IdleDet_H_Count;
		u32 CAN_IdleDet_L_Count;
		*/
		...
		*/
		...
		#if 0
		/*CAN空闲状态检测*/	
		CAN_IdleDet_L_Count=0;
		CAN_IdleDet_H_Count=0;
		do 
		{
			if ((CANx_Handle->Instance->MSR & (1<<11))==(1<<11))
			{					
				CAN_IdleDet_L_Count=0;
				CAN_IdleDet_H_Count++;
			}
			...
			delay_us((1000/125)*11);
		}while ((CAN_IdleDet_H_Count) < 128 && (CAN_IdleDet_L_Count < 128*100));
		#endif
