...
{
	char DisplayBuff[16]="星期??";
	
	
	if(Flash )//从0进入,初始化Item等值
	{
	...
	GUI_DispStringAtBar(*(Menu_Welcome[language].Text), 0, 1, 126, GUI_TA_HCENTER);
	...
	//GUI_DispStringAtBar("                ",0,0,126,GUI_TA_HCENTER);
	//GUI_DispStringAtBar("                ",0,6,126,GUI_TA_HCENTER);
	
	HAL_RTC_GetDate(&RtcHandle,&RTC_Date,RTC_FORMAT_BIN);
	my_strncpy(&DisplayBuff[4], WeekToChinese(RTC_Date.WeekDay), 3);
	GUI_DispStringAtBar((U8* )DisplayBuff,0,5,126,GUI_TA_HCENTER);

	DisplayBuff[0]='2';
	DisplayBuff[1]='0';
	snprintf(&DisplayBuff[2], 2+1,"%02d", RTC_Date.Year);
	//DisplayBuff[2]='-';
	DisplayBuff[4]=0xC4;
	DisplayBuff[5]=0xEA;

	snprintf(&DisplayBuff[6], 2+1,"%02d",RTC_Date.Month);
	//DisplayBuff[25]='-';
	DisplayBuff[8]=0xD4;
	DisplayBuff[9]=0xC2;
	
	snprintf(&DisplayBuff[10], 2+1,"%02d", RTC_Date.Date);

	DisplayBuff[12]=0xC8;
	DisplayBuff[13]=0xD5;
	DisplayBuff[14]='\0';
	GUI_DispStringAtBar((U8* )DisplayBuff,0,1,126,GUI_TA_HCENTER);

	
	HAL_RTC_GetTime(&RtcHandle,&RTC_Time,RTC_FORMAT_BIN);
	snprintf(&DisplayBuff[0], 2+1,"%02d", RTC_Time.Hours);
	//DisplayBuff[2]=':';
	DisplayBuff[2]=0xCA;
	DisplayBuff[3]=0xB1;
	snprintf(&DisplayBuff[4], 2+1,"%02d", RTC_Time.Minutes);
	//DisplayBuff[5]=':';
	DisplayBuff[6]=0xB7;
	DisplayBuff[7]=0xD6;

	
	snprintf(&DisplayBuff[8], 2+1,"%02d", RTC_Time.Seconds);
	DisplayBuff[10]=0xC3;
	DisplayBuff[11]=0xEB;
	DisplayBuff[12]='\0';


	GUI_DispStringAtBar((U8* )DisplayBuff,0,3,126,GUI_TA_HCENTER);
...
void DispIcoMenuUp()
{
    ...
    switch(Item){
        ...
        case 2:{
			...
			//时钟
            Jump2Menu(ClockSetIndex,FlashMode_AutoInit);
            return;
        }
