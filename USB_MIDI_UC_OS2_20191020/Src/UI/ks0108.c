...
/*
*****************************************************************************
* WriteCommand_ks0108 - 写命令到KS0108
* DESCRIPTION: -
* 本函数不接管片选信号
* @Param CmdData:命令码
* @Return :
...
void Display_ASCII(U8 Char, U8 X, U8 Y)
{
	...
	switch (GUIEnFont)
	{
		...
		case En_10x16:
		{
			...
			for(i=0;i<10;i++)
			{
				ImgData = chardot_10x16[i+(Char-0x30)*20];
				Display_Locate(ImgData, HZ_X, Y);
				HZ_X++;	
			}
			HZ_X=X;
		...
		case En_16x16:
		{
			...
			for(i=0;i<16;i++)
			{
				ImgData = chardot_16x16[i+(Char-0x30)*32];
				Display_Locate(ImgData, HZ_X, Y);
				HZ_X++;	
			}
			HZ_X=X;
		...
		case En_21x32:
		{
			...
			for(i=0;i<21;i++)
			{
				ImgData = chardot_21x32[i+(Char-0x30)*84];
				Display_Locate(ImgData, HZ_X, Y);
				HZ_X++;	
			}	
			HZ_X=X;
			...
			{
				ImgData = chardot_21x32[i+(Char-0x30)*84];
				Display_Locate(ImgData, HZ_X, Y + 2);
				HZ_X++;
			}
			...
			{
				ImgData = chardot_21x32[i+(Char-0x30)*84];
				Display_Locate(ImgData, HZ_X, Y + 3);
				HZ_X++;
			}
			...
			if (DP_X < LCD_XSIZE)
			{
				...
				for(i=0;i<4;i++)
				{
					//Display_Locate(0x3C, HZ_X, Y+3);
					Display_Locate(0x78, HZ_X, Y+3);
					HZ_X++;	
				}
				DP_X = LCD_XSIZE;
			}
  ...
  U8 HZ_X = X;
    ...
    switch (GUIEnFont)
	{
		...
		case En_16x16:
		{
			for(i=0;i<12;i++)
			{
				Display_Locate(0x0c, HZ_X+2, Y + 2);
				HZ_X++;
			}
		}break;
...
void GUI_SetEnFont(U8 EnFont)
{
	switch(EnFont)
	{
		...
		case En_10x16:
		{
			...
			GUIEnFont = En_10x16;
			Char_XSIZE = En_10x16_XSIZE;
			Char_YSIZE = En_10x16_YSIZE;
		}break;
		...
		case En_16x16:
		{
			...
			GUIEnFont = En_16x16;
			Char_XSIZE = En_16x16_XSIZE;
			Char_YSIZE = En_16x16_YSIZE;
		}break;
		...
		case En_21x32:
		{
			...
			GUIEnFont = En_21x32;
			Char_XSIZE = En_21x32_XSIZE;
			Char_YSIZE = En_21x32_YSIZE;
		}break;
...
/*
************************************************************************************************
*			             	                  	    		     	     Display  32x48  ASCII CODE
*
* Description : DisBitMap 
* Arguments  : 
* Returns      : void
* Notes        : 
***********************************************************************************************
...
void Clear_Locate(U8 iXpos, U8 iYpos, U8 iWide, U8 iHeight)	
{
	...
	for(iCnt= 0; iCnt< (iHeight/8) ;iCnt++)
	{
		for(iCycle= 0; iCycle< iWide; iCycle++)
		{
			Display_Locate(0x00, (iXpos + iCycle) ,iYpos);
		}
		iYpos++;
	}
}
...
void DisBitMap(_CONST_  U8 * bmp, U8 iXpos, U8 iYpos, U8 iWide, U8 iHeight)	
{
	...
	if(iYpos > (LCD_YSIZE - (iHeight /8)){//边界保护
        iYpos = (LCD_YSIZE - (iHeight /8);
    }
	
    if(iXpos >(LCD_XSIZE-16)){
        iXpos = LCD_XSIZE-16;
    }
	*/
	...
	for(iCnt= 0; iCnt< (iHeight/8) ;iCnt++)
	{
		for(iCycle= 0; iCycle< iWide; iCycle++)
		{
			Display_Locate(*bmp, (iXpos + iCycle) ,iYpos);
			bmp++;
		}
		iYpos++;
	}
...
*****************************************************************************
*/
void GUI_Draw10x16Icon(U8 _CONST_ *Ico,U8 x0,U8 y0)
{
    U8 i;
    U16 Count;
    Count = 0;
...
*****************************************************************************
*/
void GUI_Draw32x32Icon(U8 _CONST_ *Ico,U8 x0,U8 y0)
{
    U8 i;
    U16 Count;
    Count = 0;
