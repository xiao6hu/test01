 ...
 * Project:      CAN Driver for ST STM32F2xx
 * --------------------------------------------------------------------------
...
/*! \page stm32f2_can CMSIS-Driver CAN Setup
...
:-------------------|:---------------------|:------------------
CAN1 Mode           | CAN1: <b>Master</b>  | CAN1: <b>Master</b>
CAN1_RX pin         | PI9                  | PD0
CAN1_TX pin         | PH13                 | PD1
CAN2 Mode           | CAN2: <b>Slave</b>   | CAN2: <b>Slave</b>
CAN2_RX pin         | PB5                  | PB5
CAN2_TX pin         | PB13                 | PB13
 
For different boards, refer to the hardware schematics to reflect correct setup values.
 
The STM32CubeMX configuration for Keil MCBSTM32F200 Board with steps for Pinout, Clock, and System Configuration are 
...
#define ARM_CAN_DRV_VERSION ARM_DRIVER_VERSION_MAJOR_MINOR(1,11)        // CAN driver version
...
static int32_t CANx_AddFilter (uint32_t obj_idx, CAN_FILTER_TYPE filter_type, uint32_t id, uint32_t mask, uint8_t x) {
  ...
  CAN1->FMR  &= ~((uint32_t)CAN_FMR_FINIT);                             // Exit filter initialization mode
...
static int32_t CANx_SetMode (ARM_CAN_MODE mode, uint8_t x) {
  ...
  switch (mode) {
    ...
    case ARM_CAN_MODE_NORMAL:
      ...
      CAN1->FMR    &= ~((uint32_t)CAN_FMR_FINIT);        // Filter active mode
