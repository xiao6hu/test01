 ...
 * Project:      SPI Driver for ST STM32F2xx
 * --------------------------------------------------------------------------
 ...
 */
  ...
  /*! \page stm32f2_spi CMSIS-Driver SPI Setup 
...
:-------------------|:------------------------------|:-------------
SPI Mode            |SPI2: <b>Full-Duplex Master</b>|SPI2: <b>Full-Duplex Master</b>
MOSI pin            |PB15                           |PB15
MISO pin            |PB14                           |PB14
SCK pin             |PB10                           |PB10
NSS pin             |PB12                           |PB12
...
static void SPI_PeripheralReset (const SPI_TypeDef *spi) {
  ...
  if      (spi == SPI1) { __SPI1_FORCE_RESET(); }
  else if (spi == SPI2) { __SPI2_FORCE_RESET(); }
  else if (spi == SPI3) { __SPI3_FORCE_RESET(); }

  __NOP(); __NOP(); __NOP(); __NOP(); 

  if      (spi == SPI1) { __SPI1_RELEASE_RESET(); }
  else if (spi == SPI2) { __SPI2_RELEASE_RESET(); }
  else if (spi == SPI3) { __SPI3_RELEASE_RESET(); }
}
...
#ifdef RTE_DEVICE_FRAMEWORK_CLASSIC
      // Disable SPI clock
      if      (spi->reg == SPI1) { __SPI1_CLK_DISABLE(); }
      else if (spi->reg == SPI2) { __SPI2_CLK_DISABLE(); }
      else if (spi->reg == SPI3) { __SPI3_CLK_DISABLE(); }
#endif
...
#ifdef RTE_DEVICE_FRAMEWORK_CLASSIC
      // Enable SPI clock
      if      (spi->reg == SPI1) { __SPI1_CLK_ENABLE(); }
      else if (spi->reg == SPI2) { __SPI2_CLK_ENABLE(); }
      else if (spi->reg == SPI3) { __SPI3_CLK_ENABLE(); }
