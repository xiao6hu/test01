...
{
	UNS32 TPDO1_COBID = 0x180; 
	UNS32 RPDO1_COBID = 0x27f;
	UNS32 TPDO2_COBID = 0x180; 
	UNS32 RPDO2_COBID = 0x200;
	...
	TpdoTransmissionType = TRANS_EVENT_PROFILE;
	writeLocalDict( d, /*CO_Data* d*/
			0x1400, /*UNS16 index*/
			0x02, /*UNS8 subind*/ 
...
void Master_operational(CO_Data* d)
{
	...
	setTimer(100);
	//TIM_Cmd(TIM4, ENABLE);
	__HAL_TIM_ENABLE(&TIM4_HandleStructure);
...
void SetCanHandle(CO_Data* d, CAN_TypeDef* CANx)
{
	d->canHandle = CANx;
}
